...
ggplot.calibration <- function(data, ..., bwidth = 2, dwidth = 3){
  ...
  if(mods == 1) {
    out <- ggplot(data$data, aes(x = midpoint, y = Percent)) +
      geom_abline(slope = 1, intercept = 0, col = "#9ecae1", lty = 2, alpha = .3) +
      geom_line(size = 1.2, col = "#9ecae1") +
      geom_point(size = 2, col = "#9ecae1") +
    ...
    out <- ggplot(data$data, aes(x = midpoint, y = Percent,
      ...
      geom_abline(slope = 1, intercept = 0, col = "#9ecae1", lty = 2, alpha = .3) +
      geom_line(size = 1.2, position = position_dodge(width = dwidth)) +
      geom_point(size = 2, position = position_dodge(width = dwidth)) +
      geom_errorbar(aes(ymin = Lower, ymax = Upper), 
                    width = bwidth,
                    size = 1.1,
